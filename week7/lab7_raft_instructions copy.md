...
### A2b. (Only if bridge networking fails) Alternative: `--net=host` setup

Skip this section if A2 worked. If your containers start but the cluster never becomes healthy — you see repeated `i/o timeout` connecting to peer ports, and `ping` between containers drops 100% of packets — your system's `iptables FORWARD` policy is likely blocking bridge traffic. This can happen when `ufw` is active without Docker-friendly rules, or on certain non-VM server environments. Run the following diagnostic first:

```bash
docker run --rm --network etcd-net alpine ping -c 2 etcd1 2>&1 | tail -3
```

If you see `100% packet loss`, the bridge approach will not work. Tear down and use `--net=host` instead:

```bash
# Tear down the bridge-based setup
docker stop etcd1 etcd2 etcd3 2>/dev/null; docker rm etcd1 etcd2 etcd3 2>/dev/null; docker network rm etcd-net 2>/dev/null; true

# Re-export CLUSTER_EP to use 127.0.0.1 addresses
export CLUSTER_EP=http://127.0.0.1:2379,http://127.0.0.1:2381,http://127.0.0.1:2382

# Launch three nodes on the host network with distinct ports
docker run -d --name etcd1 --net=host \
  ${ETCD_IMAGE} etcd --name node1 --data-dir /etcd-data \
  ...
  --initial-cluster node1=http://127.0.0.1:2380,node2=http://127.0.0.1:2384,node3=http://127.0.0.1:2388 \
  --initial-cluster-state new --initial-cluster-token lab7-cluster

docker run -d --name etcd2 --net=host \
  ${ETCD_IMAGE} etcd --name node2 --data-dir /etcd-data2 \
  --listen-client-urls http://127.0.0.1:2381 --advertise-client-urls http://127.0.0.1:2381 \
  --listen-peer-urls http://127.0.0.1:2384 --initial-advertise-peer-urls http://127.0.0.1:2384 \
  --initial-cluster node1=http://127.0.0.1:2380,node2=http://127.0.0.1:2384,node3=http://127.0.0.1:2388 \
  --initial-cluster-state new --initial-cluster-token lab7-cluster

docker run -d --name etcd3 --net=host \
  ${ETCD_IMAGE} etcd --name node3 --data-dir /etcd-data3 \
  --listen-client-urls http://127.0.0.1:2382 --advertise-client-urls http://127.0.0.1:2382 \
  --listen-peer-urls http://127.0.0.1:2388 --initial-advertise-peer-urls http://127.0.0.1:2388 \
  --initial-cluster node1=http://127.0.0.1:2380,node2=http://127.0.0.1:2384,node3=http://127.0.0.1:2388 \
  --initial-cluster-state new --initial-cluster-token lab7-cluster
```

**`--net=host` endpoint mapping** — whenever a command below uses a container DNS name as an endpoint, substitute the corresponding `127.0.0.1` address:

| Container name endpoint | `--net=host` substitute |
|-------------------------|------------------------|
| `http://etcd1:2379` | `http://127.0.0.1:2379` |
| `http://etcd2:2379` | `http://127.0.0.1:2381` |
| `http://etcd3:2379` | `http://127.0.0.1:2382` |
| `http://etcd-single:2379` | `http://127.0.0.1:2390` |

`CLUSTER_EP` was already re-exported above, so commands using `${CLUSTER_EP}` work unchanged. Commands that **hardcode** container DNS names (e.g. `--endpoints=http://etcd2:2379,http://etcd3:2379` in Part B, or `--endpoints=http://etcd1:2379,http://etcd2:2379` in Part F) must be substituted manually using the table above.

**Part C — single-node container** (`--net=host` variant):

```bash
docker run -d --name etcd-single --net=host \
  ${ETCD_IMAGE} etcd --name single --data-dir /etcd-data-single \
  --listen-client-urls http://127.0.0.1:2390 --advertise-client-urls http://127.0.0.1:2390 \
  --listen-peer-urls http://127.0.0.1:2391 --initial-advertise-peer-urls http://127.0.0.1:2391
```

Also add `--endpoints=${SINGLE_EP}` to every `etcdctl_in etcd-single ...` command in Part C Phase 1 (the default endpoint is `127.0.0.1:2379`, which is etcd1, not etcd-single).

**Part E — network partition** (`--net=host` variant): `docker network disconnect/connect` has no effect on `--net=host` containers. Use `docker pause/unpause` to simulate an isolated node instead:
...
`docker pause` suspends all processes in the container. The majority side (etcd1 + etcd2) continues making progress; the paused node cannot send or receive any messages, which is sufficient to observe the minority-cannot-commit property.
...
* **all three endpoints unhealthy / `i/o timeout` on peer port / 100% ping loss between containers**: your `iptables FORWARD` policy is blocking Docker bridge traffic (common when `ufw` is active). See section **A2b** for the `--net=host` alternative setup.
